...
class SARAS(nn.Module):
    ...
    def __init__(self, num_classes=2, use_aux=True, fpn_out=48, freeze_bn=False, **_):
        ...
        self.output_fill = nn.Sequential(
            # nn.ConvTranspose2d(fpn_out, fpn_out, kernel_size=2, stride = 2, bias=False),    # levir, WHU
            # nn.ConvTranspose2d(fpn_out , fpn_out, kernel_size=3, stride =1, bias=False, padding=1),
            nn.Conv2d(fpn_out, fpn_out, kernel_size=3, stride=1, padding=1, padding_mode='replicate', bias=False),
            nn.BatchNorm2d(fpn_out),
            nn.ReLU(inplace=True),
