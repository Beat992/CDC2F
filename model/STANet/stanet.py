...
from model.STANet import resnet
from model.STANet._blocks import Conv1x1, Conv3x3, get_norm_layer
from model.STANet._utils import KaimingInitMixin, Identity
...
class STANet(nn.Module):
    ...
    def forward(self, t1, t2):
        ...
        b = f1.shape[0]
        h = f1.shape[2]
        f1 = f1.view(b, self.width, -1).transpose(1, 2)
        f2 = f2.view(b, self.width, -1).transpose(1, 2)
        dist = self.calc_dist(f1, f2).reshape(b, 1, h, h)
