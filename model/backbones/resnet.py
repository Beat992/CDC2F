...
from configs.LEVIR_CD import pretrain_model_path_ResNet18, pretrain_model_path_ResNet34, pretrain_model_path_ResNet50
...
class ResNet(nn.Module):
    ...
    # resnet50 block = bottlenect layers = [3, 4, 6, 3]
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        if arch == 'resnet18':
            state_dict = torch.load(pretrain_model_path_ResNet18)
        elif arch == 'resnet34':
            state_dict = torch.load(pretrain_model_path_ResNet34)
        elif arch == 'resnet50':
            state_dict = torch.load(pretrain_model_path_ResNet50)
        else:
            raise 'Just support resnet 18/34/50 as backbone'
